...
class PipelineOrchestrator:
    ...
    def _unregister_hooks(self, handlers: tuple):
        """注销进度回调钩子"""
    ...
    async def run_pipeline(self, requirement: str,
                           progress_cb: Callable[[PipelineEvent], None] = None) -> dict:
        """
        同步执行流水线（等待完成）。
    ...
    async def _wait_for_result(self, run_id: str, timeout: int = 300):
        """等待 WorkflowEngine 执行完成"""
        from tools.workflow.engine import WorkflowResult
        ...
        while elapsed < timeout:
            ...
            if result and result.status != "running":
                return {
                    ...
                    "logs": [
                        {"node_id": l.node_id, "status": l.status, "duration_ms": l.duration_ms}
